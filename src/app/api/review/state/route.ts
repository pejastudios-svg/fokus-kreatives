  ...
  try {
    ...
    const [itemsRes, commentsRes, assigneesRes] = await Promise.all([
      ...
      reviewAdmin
        ...
        .select(
          'id, approval_item_id, content, created_at, updated_at, user_id, reviewer_email, attachments, resolved, file_url, file_name, timestamp_seconds, region, attachment_index, parent_comment_id, users:user_id (name, email, profile_picture_url)',
        )
        .eq('approval_id', approval.id)
