...
import { buildCarouselPrompt, type PackagePillar } from '@/lib/prompt/packagePrompt'
import { validateCarouselStructure, formatIssuesForRetry } from '@/lib/prompt/validator'
...
export async function POST(req: NextRequest) {
  try {
    ...
    let issues = validateCarouselStructure(output)
    if (issues.length) {
      const retryPrompt = `${prompt.user}\n\n${formatIssuesForRetry(issues)}`
      const retried = await runOnce(retryPrompt)
      const retriedIssues = validateCarouselStructure(retried)
    ...
    console.error('package carousel error:', err)
